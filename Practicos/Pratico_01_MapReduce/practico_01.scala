...
(B, [A, C, D, E]) ==> ({B, A}, [A, C, D, E])
                      ({B, C}, [A, C, D, E])
                      ({B, D}, [A, C, D, E])
                      ({B, E}, [A, C, D, E])

(C, [A, B, D, E]) ==> ({C, A}, [A, B, D, E])
                      ({C, B}, [A, B, D, E])
                      ({C, D}, [A, B, D, E])
                      ({C, E}, [A, B, D, E])

(D, [A, B, C, E]) ==> ({D, A}, [A, B, C, E])
                      ({D, B}, [A, B, C, E])
                      ({D, C}, [A, B, C, E])
                      ({D, E}, [A, B, C, E])
...
def promedio (nums: List[Double]) : Double = {
    ...
    val fmap = (_ :Unit, n: Double) => List(("key", n))
    val freduce = (key: String, vs: List[Double]) => (key, vs.fold (0.0) (_+_))

    val mr = mapReduce (datos) (fmap) (freduce)

    //> mr
    //> List[(String, Double)] = List((key, promedio))
    return mr(0)._2 / len_list
}
